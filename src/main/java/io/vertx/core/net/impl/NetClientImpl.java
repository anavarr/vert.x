...
public class NetClientImpl implements MetricsProvider, NetClient, Closeable {
   ...
   * @param useHybrid wether to use Hybrid Key Exchange Protocol (along with SSL)
  ...
  public void connectInternal(ProxyOptions proxyOptions,
    ...
    if (closeFuture.isClosed()) {
      ...
      fut.onComplete(ar -> {
        if (ar.succeeded()) {
          connectInternal2(proxyOptions, remoteAddress, peerAddress, ar.result().sslChannelProvider(), serverName, ssl, useAlpn, useHybrid, registerWriteHandlers, connectHandler, context, remainingAttempts);
        } else {
  ...
  private void connectInternal2(ProxyOptions proxyOptions,
    ...
    if (eventLoop.inEventLoop()) {
      ...
      io.netty.util.concurrent.Future<Channel> fut = channelProvider.connect(remoteAddress, peerAddress, serverName, ssl, useAlpn, useHybrid);
          ...
          if (connectError && (remainingAttempts > 0 || remainingAttempts == -1)) {
            context.emit(v -> {
              ...
              vertx.setTimer(options.getReconnectInterval(), tid ->
                connectInternal(proxyOptions, remoteAddress, peerAddress, serverName, ssl, useAlpn, useHybrid, registerWriteHandlers, connectHandler, context, remainingAttempts == -1 ? remainingAttempts : remainingAttempts - 1)
              );
            });
          } else {
      ...
      eventLoop.execute(() -> connectInternal2(proxyOptions, remoteAddress, peerAddress, sslChannelProvider, serverName, ssl, useAlpn, useHybrid, registerWriteHandlers, connectHandler, context, remainingAttempts));
    }
  }
